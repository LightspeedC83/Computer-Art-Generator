...
for point in points:
  ...
  if fancy_generation:
    ...
    shapes.append(correct_order)
  else:
    shapes.append(current_vertices)

shape_index = 0  #this was a failed attempt to get the program to draw images faster
for shape in shapes:
  t = turtle.Turtle()
  t.speed(10000)
  turtle.hideturtle()
  if fancy_generation:
    ...
    t.goto((shape[0][0], shape[0][1])) #drawing it
    t.begin_fill() 
    for vertex in shape:
  ...
  #the alternative method for drawing the shape, will produce more spidery images
    ...
    t.penup()
    t.goto((shape[0][0], shape[0][1])) #drawing it as it appeared when gathered as points closest to one main point
    t.begin_fill()
    for vertex in shape:
      t.pendown()
      t.goto(vertex[0], vertex[1])
